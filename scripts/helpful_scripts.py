...
def get_contract(contract_name):
    ...
    otherwise it iwll deploy a mock version of that contract and return that mock contract
...
def deploy_mocks(decimals=DECIMALS, initial_value=INITIAL_VALUE):
    ...
    vrf_coordinator_mock = VRFCoordinatorMock.deploy(link_token.address, {"from": account})
    print("Deployed Mock!")
...
def fund_with_link(contract_address, account=None, link_token=None, amount=100000000000000000):
    ...
    #link_token_address = interface.LinkTokenInterface(link_token.address)
    #tx = link_token_address.transfer(contract_address, amount, {"from": account})
    tx.wait(1)
    print("The contract has been founded with link.")
    return tx
